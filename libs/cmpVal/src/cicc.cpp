...
static void addProfileFunctionCallInt(Instruction* BI, Module* module){
	...
	if (BI->getOperand(0)->getType()->isIntegerTy(64) != true)
	{
	    // Convert i32 to i64
		SExtInst* convertInst1 = new SExtInst(BI->getOperand(0), Type::getInt64Ty(module->getContext()), "convt1", instPos);
		SExtInst* convertInst2 = new SExtInst(BI->getOperand(1), Type::getInt64Ty(module->getContext()), "convt1", instPos);
    ...
    else
    {
		...
		checker_args[0] = BI->getOperand(0);
		checker_args[1] = BI->getOperand(1);
		checker_args[2] = indexValue;
		checker_arg_types[0] = BI->getOperand(0)->getType();
		checker_arg_types[1] = BI->getOperand(1)->getType();
		checker_arg_types[2] = indexValue->getType();  
    }
	...
	Constant* checker_handler_c = module->getOrInsertFunction("profileICmpValue", checker_type);
	Function* checker_handler = dyn_cast<Function>(checker_handler_c);
	CallInst::Create(checker_handler, args, "", instPos);
}

static void addProfileFunctionCallFloat(Instruction* BI, Module* module){
	...
	if (BI->getOperand(0)->getType()->isDoubleTy() != true)
	{
	    // Convert float to double
		FPExtInst* convertInst1 = new FPExtInst(BI->getOperand(0), Type::getDoubleTy(module->getContext()), "convt1", instPos);
		FPExtInst* convertInst2 = new FPExtInst(BI->getOperand(1), Type::getDoubleTy(module->getContext()), "convt1", instPos);
		checker_args[0] = convertInst1;
		checker_args[1] = convertInst2;
		checker_args[2] = indexValue;
		checker_arg_types[0] = convertInst1->getType();
	...
	FunctionType* checker_type = FunctionType::get(Type::getVoidTy(BI->getContext()), argsTypes, false);
	Constant* checker_handler_c = module->getOrInsertFunction("profileFCmpValue", checker_type);
	Function* checker_handler = dyn_cast<Function>(checker_handler_c);
	CallInst::Create(checker_handler, args, "", instPos);
}
...
static void addProfileDump(Instruction* BI, Module* module){
	...
	ArrayRef<Type*> argsTypes(checker_arg_types);
	FunctionType* checker_type = FunctionType::get(Type::getVoidTy(BI->getContext()), argsTypes, false);
	Constant* checker_handler_c = module->getOrInsertFunction("dumpResult", checker_type);
	Function* checker_handler = dyn_cast<Function>(checker_handler_c);
	...
	for (Module::iterator F = module->begin(), e = module->end(); F != e; F++){
		...
		for(Function::iterator BB = F->begin(), E = F->end(); BB != E; ++BB){
			for(BasicBlock::iterator BI = BB->begin(), BE = BB->end(); BI != BE; ++BI) {
				...
				if(getBambooIndex(BI) != -1 && opcode == 46){
					if(BI->getOperand(0)->getType()->isIntegerTy() == true){
						addProfileFunctionCallInt(BI, module);
					}
				}

				if(getBambooIndex(BI) != -1 && opcode == 47){
					addProfileFunctionCallFloat(BI, module);
				}
			}
		}
	}	

	errs() << "Compare instruction Value pass installed ... \n"; 
