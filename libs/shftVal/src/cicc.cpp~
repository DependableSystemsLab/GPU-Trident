...
static void insertForShift(Instruction* shiftInst, Module* module){

    BasicBlock::iterator nextInst = shiftInst;
	nextInst++;
	Instruction* instPos = nextInst;
	Value* indexValue = ConstantInt::get(Type::getInt64Ty(shiftInst->getContext()), getBambooIndex(shiftInst));
	...
	{
		// Convert i32 to i64
		SExtInst* convertInst1 = new SExtInst(shiftInst->getOperand(1), Type::getInt64Ty(module->getContext()), "convt1", instPos);
    ...
    Constant* checker_handler_c = module->getOrInsertFunction("profileShiftValues", checker_type);
	...
	for (Module::iterator F = module->begin(), e = module->end(); F != e; F++){
		...
		for(Function::iterator BB = F->begin(), E = F->end(); BB != E; ++BB){
			for(BasicBlock::iterator BI = BB->begin(), BE = BB->end(); BI != BE; ++BI) {
				...
				if( getBambooIndex(BI) != -1 && opcode == 21 || opcode == 22 || opcode == 23)
				{
					insertForShift(BI, module);
				}
			}
		}
	}	
