...
/* JUSTIN: You can remove cpu part as we never need it */
...
int main(void) { 
    ...
    /* JUSTIN: Make sure you do not use random values here and fix input values for each kernel. */
    // Initialize matrices with random numbers
    srand(time(NULL));
    for (int i = 0; i < N; i++) { 
        for(int j = 0 ; j < N; j++) {

            int valA = (((i + j) % 10) == 0) ? 1 : (i + j) % 10;
            a[i * N + j] = valA;
            int valB = ((j % 10) == 0) ? 1 : (j % 10);
            b[i * N + j] = valB;
            c[i * N + j] = 0;
            d[i * N + j] = 0;
        }
    ...
    bambooLogKernelBegin();
    gpu_mult<<<Grid, Block>>>(dev_a, dev_b, dev_c, N);
    bambooLogKernelEnd();
    
    cudaMemcpy(c, dev_c, size, cudaMemcpyDeviceToHost);
    /* JUSTIN: Dump values in c to a text file. */
    /* After every FI run, compare the values in c that just dumped with the one from golden run. If they are the same, it's benign. Otherwise, it' SDC if it is not a crash. That way we say we are examining memory values manipulated by the kernel. */
