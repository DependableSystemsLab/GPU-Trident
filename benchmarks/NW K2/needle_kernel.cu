  ...
  __syncthreads();
    ...
    for( int m = 0 ; m < BLOCK_SIZE ; m++){
        ...
        if ( tx <= m ){
            ...
            if( temp[t_index_y-1][t_index_x-1] + ref[t_index_y-1][t_index_x-1] <= temp[t_index_y][t_index_x-1]  - penalty )
                k = temp[t_index_y][t_index_x-1]  - penalty;
            else 
                k = temp[t_index_y-1][t_index_x-1] + ref[t_index_y-1][t_index_x-1];

            if( k <= temp[t_index_y-1][t_index_x]  - penalty)
                    temp[t_index_y][t_index_x] = (temp[t_index_y-1][t_index_x]  - penalty);
            else
                    temp[t_index_y][t_index_x] = k;
        }
