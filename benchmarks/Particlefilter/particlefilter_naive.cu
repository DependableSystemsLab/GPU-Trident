...
__global__ void kernel(double * arrayX, double * arrayY, double * CDF, double * u, double * xj, double * yj, int Nparticles){
    ...
    int i = blockDim.x * block_id + threadIdx.x; // %5
...
void particleFilter(int * I, int IszX, int IszY, int Nfr, int * seed, int Nparticles){
    ...
    for(k = 1; k < Nfr; k++){
        ...
        // JUSTIN: dump to result.txt
        FILE *fpo_result = fopen("result.txt","a");
        fprintf(fpo_result,"XE: %lf\n", xe);
        fprintf(fpo_result,"YE: %lf\n", ye);
        // END
        ...
        // JUSTIN: dump to result
        fprintf(fpo_result,"%lf\n", distance);
        fclose(fpo_result);
        // END      
